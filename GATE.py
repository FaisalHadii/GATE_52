...
# --- GLOBAL VARIABLES FOR CONSISTENT AXIS SCALING ---
...
def determine_global_axis_limits(base_output_dir: Path, thicknesses_cm: list):
    """
    Analyzes all phase space data to determine global x and y axis limits
    for consistent scaling across all plots.
    """
    ...
    for thickness in thicknesses_cm:
        # Check TOPAS data
        topas_dir = base_output_dir / f"topas_run_{thickness:.4f}cm"
        topas_phsp_file = topas_dir / "MyTopasOutput.phsp"
        
        if topas_phsp_file.exists():
            try:
                df = read_topas_phasespace(topas_phsp_file)
                if df is not None and not df.empty:
                    if 'Position_X' in df.columns:
                        x_pos = df['Position_X'] * 10  # Convert to mm
                        y_pos = df['Position_Y'] * 10  # Convert to mm
                    else:
                        x_pos = df['X_cm'] * 10  # Convert to mm
                        y_pos = df['Y_cm'] * 10  # Convert to mm
                    
                    all_x_values.extend(x_pos.values)
                    all_y_values.extend(y_pos.values)
            except Exception as e:
                print(f"Warning: Could not read TOPAS phase space for axis limits: {e}")
        
        # Check GATE data
        gate_dir = base_output_dir / f"gate_run_{thickness:.4f}cm"
        gate_phsp_file = gate_dir / "phantom_transmission_TEMPLATE.root"
        
        if gate_phsp_file.exists():
            try:
                df = read_gate_phasespace(gate_phsp_file)
                if df is not None and not df.empty:
                    x_pos = df['X_cm'] * 10  # Convert to mm
                    y_pos = df['Y_cm'] * 10  # Convert to mm
                ...
                print(f"Warning: Could not read GATE phase space for axis limits: {e}")
    ...
    if all_x_values and all_y_values:
        ...
        GLOBAL_X_LIMITS = (x_min - x_padding, x_max + x_padding)
        GLOBAL_Y_LIMITS = (y_min - y_padding, y_max + y_padding)
        
        print(f"Global axis limits determined:")
        print(f"X-axis: {GLOBAL_X_LIMITS[0]:.2f} to {GLOBAL_X_LIMITS[1]:.2f} mm")
        print(f"Y-axis: {GLOBAL_Y_LIMITS[0]:.2f} to {GLOBAL_Y_LIMITS[1]:.2f} mm")
    else:
        # Fallback to reasonable defaults
        GLOBAL_X_LIMITS = (-150, 150)  # mm
        GLOBAL_Y_LIMITS = (-150, 150)  # mm
        print("Using default axis limits: ±150 mm")

# --- The rest of the functions (run_topas_simulation, run_gate_simulation, etc.) remain unchanged. ---
...
def plot_scatter_on_ax(ax, df: pd.DataFrame, title: str):
    """Helper function to plot scatter data on a given axis with consistent scaling."""
    if df is None or df.empty:
        ...
        if GLOBAL_X_LIMITS and GLOBAL_Y_LIMITS:
...
def main():
    ...
    determine_global_axis_limits(base_output_dir, thicknesses_cm)
