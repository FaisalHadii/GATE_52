...
def read_topas_phasespace(phsp_file: Path) -> pd.DataFrame | None:
    ...
    try:
        ...
        # TOPAS positions are in cm, so create '_cm' columns
...
def read_gate_phasespace(phsp_file: Path) -> pd.DataFrame | None:
    ...
    try:
        with uproot.open(phsp_file) as file:
            ...
            # GATE positions ('X', 'Y') are in mm. Convert to cm upon reading.
            df['X_cm'] = df['X'] / 10.0
            df['Y_cm'] = df['Y'] / 10.0
...
def plot_scatter_on_ax(ax, df: pd.DataFrame, title: str):
    ...
    # Unified logic: Both GATE and TOPAS dataframes now have 'X_cm' and 'Y_cm'.
    # Always convert from cm to mm for plotting.
    x_pos = df['X_cm'] * 10
    y_pos = df['Y_cm'] * 10
...
def plot_crosssection_sum_comparison(topas_df, gate_df, thickness, output_dir):
    ...
    topas_x_sum = topas_df.groupby(topas_df['X_mm'].round().astype(int)).size()
    gate_x_sum = gate_df.groupby(gate_df['X_mm'].round().astype(int)).size()
    topas_y_sum = topas_df.groupby(topas_df['Y_mm'].round().astype(int)).size()
    gate_y_sum = gate_df.groupby(gate_df['Y_mm'].round().astype(int)).size()

    fig, axs = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle(f'Cross-Section Sums Comparison (Thickness: {thickness} cm)', fontsize=16, weight='bold')

    axs[0].plot(topas_x_sum.index, topas_x_sum.values, 'o-', label='TOPAS', alpha=0.8)
    axs[0].plot(gate_x_sum.index, gate_x_sum.values, 's--', label='GATE', alpha=0.8)
    axs[0].set_xlabel('X Position (mm)')
    axs[0].set_ylabel('Sum over Y (particle count)')
    axs[0].set_title('Total over Y for each X')
    axs[0].legend()
    axs[0].grid(True, linestyle='--', alpha=0.6)

    axs[1].plot(topas_y_sum.index, topas_y_sum.values, 'o-', label='TOPAS', alpha=0.8)
    axs[1].plot(gate_y_sum.index, gate_y_sum.values, 's--', label='GATE', alpha=0.8)
    axs[1].set_xlabel('Y Position (mm)')
    axs[1].set_ylabel('Sum over X (particle count)')
    axs[1].set_title('Total over X for each Y')
    axs[1].legend()
...
def plot_xy_cross_sections_separate(topas_df: pd.DataFrame | None, gate_df: pd.DataFrame | None, thickness: float, output_dir: Path):
    """
    Create separate figures for total cross-sections along X (sum over Y) and along Y (sum over X),
    comparing TOPAS vs GATE for the given thickness.
    """
    if (topas_df is None or topas_df.empty) and (gate_df is None or gate_df.empty):
        print(f"Skipping separate X/Y cross-sections for thickness {thickness} cm due to missing data.")
        return

    def axis_sum(df: pd.DataFrame, which: str) -> pd.Series:
        if df is None or df.empty:
            return pd.Series(dtype=int)
        df_local = df.copy()
        df_local['X_mm'] = df_local['X_cm'] * 10
        df_local['Y_mm'] = df_local['Y_cm'] * 10
        if which == 'x':
            return df_local.groupby(df_local['X_mm'].round().astype(int)).size()
        if which == 'y':
            return df_local.groupby(df_local['Y_mm'].round().astype(int)).size()
        raise ValueError("which must be 'x' or 'y'")

    # X-direction (sum over Y)
    topas_x = axis_sum(topas_df, 'x')
    gate_x = axis_sum(gate_df, 'x')
    x_index = sorted(set(topas_x.index.tolist()) | set(gate_x.index.tolist()))
    topas_x = topas_x.reindex(x_index).fillna(0)
    gate_x = gate_x.reindex(x_index).fillna(0)

    plt.figure(figsize=(12, 5))
    plt.title(f'Total X Cross-Section (sum over Y) - {thickness} cm', fontsize=14, weight='bold')
    plt.plot(x_index, topas_x.values, label='TOPAS', alpha=0.85)
    plt.plot(x_index, gate_x.values, label='GATE', alpha=0.85, linestyle='--')
    plt.xlabel('X Position (mm)')
    plt.ylabel('Total count (sum over Y)')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
    out_x = output_dir / f"crosssection_X_total_{thickness:.4f}cm.png"
    plt.tight_layout()
    plt.savefig(out_x, dpi=300)
    plt.close()
    print(f"✅ X-direction total cross-section saved to: {out_x}")

    # Y-direction (sum over X)
    topas_y = axis_sum(topas_df, 'y')
    gate_y = axis_sum(gate_df, 'y')
    y_index = sorted(set(topas_y.index.tolist()) | set(gate_y.index.tolist()))
    topas_y = topas_y.reindex(y_index).fillna(0)
    gate_y = gate_y.reindex(y_index).fillna(0)

    plt.figure(figsize=(12, 5))
    plt.title(f'Total Y Cross-Section (sum over X) - {thickness} cm', fontsize=14, weight='bold')
    plt.plot(y_index, topas_y.values, label='TOPAS', alpha=0.85)
    plt.plot(y_index, gate_y.values, label='GATE', alpha=0.85, linestyle='--')
    plt.xlabel('Y Position (mm)')
    plt.ylabel('Total count (sum over X)')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
    out_y = output_dir / f"crosssection_Y_total_{thickness:.4f}cm.png"
    plt.tight_layout()
    plt.savefig(out_y, dpi=300)
    plt.close()
    print(f"✅ Y-direction total cross-section saved to: {out_y}")
...
def main():
    ...
    for thickness in thicknesses_cm:
        ...
        # Uncomment to run simulations on-demand
    ...
    for thickness in thicknesses_cm:
        ...
        if topas_df is not None and not topas_df.empty:
            ...
            topas_dfs[thickness] = pd.DataFrame()
        ...
        # Total counts per axis (X: sum over Y, Y: sum over X) - combined figure
        plot_crosssection_sum_comparison(
            current_topas_df, current_gate_df, thickness, base_output_dir
        )

        # Total counts per axis saved as separate figures (explicit X and Y cross-sections)
        plot_xy_cross_sections_separate(
            current_topas_df, current_gate_df, thickness, base_output_dir
        )

        # Heatmap percentage differences
