...
# --- TOPAS CONFIGURATION ---
TOPAS_EXECUTABLE_PATH = os.environ.get("TOPAS_EXECUTABLE_PATH", "/home/fhhadi/topas/bin/topas")
BASE_MACRO_FILE = os.environ.get("TOPAS_BASE_MACRO_FILE", "compined1.txt")

# --- GATE CONFIGURATION ---
# If your Gate executable is on PATH, set to "gate". Otherwise, set GATE_EXECUTABLE_PATH env var.
GATE_EXECUTABLE_PATH = os.environ.get("GATE_EXECUTABLE_PATH", "gate")
# Base Gate macro to execute. Provide a valid path via env var if different
GATE_BASE_MACRO_FILE = os.environ.get("GATE_BASE_MACRO_FILE", "base_gate.mac")
# Command template to set thickness in your Gate macro. Adjust to your macro.
# Common pattern is to use an alias in the base macro, e.g., ${THICKNESS}.
GATE_THICKNESS_CMD_TEMPLATE = os.environ.get(
    "GATE_THICKNESS_CMD_TEMPLATE", "/control/alias THICKNESS {thickness_cm} cm"
)
# Optional: alias for outputs directory used by your Gate macro (if applicable)
GATE_OUTPUT_DIR_ALIAS = os.environ.get("GATE_OUTPUT_DIR_ALIAS", "OUTPUT_DIR")
...
def modify_topas_macro(template_path: Path, output_path: Path, phantom_half_thickness_cm: float, output_dir: Path):
    ...
    print(f"Created temporary TOPAS macro for thickness {phantom_half_thickness_cm * 2} cm.")
...
def modify_gate_macro(gate_template_path: Path, output_path: Path, full_thickness_cm: float, output_dir: Path) -> bool:
    """Creates a wrapper Gate macro that sets thickness/output aliases and executes the base macro.

    This assumes your base Gate macro reads aliases like ${THICKNESS} and possibly ${OUTPUT_DIR}.
    Adjust via env vars GATE_THICKNESS_CMD_TEMPLATE and GATE_OUTPUT_DIR_ALIAS if needed.
    """
    if not gate_template_path.exists():
        print(f"Gate base macro not found: {gate_template_path}. Skipping Gate run.")
        return False

    lines: list[str] = []
    lines.append("# Auto-generated wrapper for Gate run")
    thickness_cmd = GATE_THICKNESS_CMD_TEMPLATE.format(thickness_cm=full_thickness_cm)
    if thickness_cmd:
        lines.append(thickness_cmd)
    if GATE_OUTPUT_DIR_ALIAS:
        # Alias output directory for the base macro to use if it wants
        lines.append(f"/control/alias {GATE_OUTPUT_DIR_ALIAS} \"{output_dir}\"")
    # Execute the user-provided base macro last
    lines.append(f"/control/execute \"{str(gate_template_path)}\"")

    output_path.write_text("\n".join(lines))
    print(f"Created temporary Gate macro for thickness {full_thickness_cm} cm.")
    return True


def run_gate_simulation(macro_path: Path) -> bool:
    """Executes a Gate simulation."""
    print("-" * 60, f"\nRunning Gate for: {macro_path.name}\n", "-" * 60)
    try:
        subprocess.run([GATE_EXECUTABLE_PATH, str(macro_path)], check=True, capture_output=True, text=True)
...
def main():
    """Main function to orchestrate the Gate + TOPAS simulation and analysis workflow."""
    ...
    for thickness in thicknesses_cm:
        ...
        # --- Run Gate first (optional, if macro exists) ---
        gate_base_macro_path = Path(GATE_BASE_MACRO_FILE)
        gate_macro_path = run_output_dir / "gate_run.mac"
        if modify_gate_macro(gate_base_macro_path, gate_macro_path, thickness, run_output_dir):
            _ = run_gate_simulation(gate_macro_path)
        else:
            print("Gate run skipped for this thickness.")

        # --- Run TOPAS ---
        temp_macro_path = run_output_dir / "topas_run.txt"
        try:
            modify_topas_macro(Path(BASE_MACRO_FILE), temp_macro_path, half_thickness_cm, run_output_dir)
        except FileNotFoundError as e:
            print(str(e))
            print(f"Skipping TOPAS run for thickness {thickness} cm due to missing macro file.")
            temp_macro_path = None

        if temp_macro_path and run_topas_simulation(temp_macro_path):
